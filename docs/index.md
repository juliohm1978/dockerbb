...
Para que funcione, vários processos são gerenciados dentro do container pelo `/sbin/init`, comum em várias distribuições. É como se uma mini-estação de trabalho estivesse executando dentro do container. Isto quebra o paradigma "*um processo por container*", mas faz-se necessário nos moldes desta solução.
